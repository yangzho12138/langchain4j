...
class GeminiService {
    ...
    GeminiGenerateContentResponse generateContent(
            String modelName, String apiKey, GeminiGenerateContentRequest request) {
    ...
    private <T> Stream<T> streamRequest(String url, String apiKey, Object requestBody, Class<T> responseType) {
        try {
            ...
            Stream<T> responseStream = httpResponse
                    .body()
            ...
            if (logger != null) {
                responseStream =
                        responseStream.peek(response -> logger.debug("Partial response from Gemini:\n{}", response));
            }
